...
int readSocketIntoFile(const int socket, const char * const pathname, const int lengthBytes) {
	...
	const int buffer_max_size = 64 * 1024;
	char buffer[buffer_max_size];

	while (totalBytesRead < lengthBytes) {
		const int bytesRemaining = lengthBytes - totalBytesRead;
		// Try for smaller of buffer_max_size or bytesRemaining
		int bytesToAttemptRead;
		if(bytesRemaining < buffer_max_size) {
			bytesToAttemptRead = bytesRemaining;
		} else {
			bytesToAttemptRead = buffer_max_size;
		}
		
		const int bytesReceived = recv(socket, buffer, bytesToAttemptRead, 0);

		// Write data from buffer starting at beginning of buffer until bytesReceived length into file ptr "fh"
		// 1 means size is 1 byte with a total number members of bytesReceived (write the number of bytes just received)
